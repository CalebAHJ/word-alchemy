...
function char_check(word, guy, girl) {
    ...
    let total_chars = guy_chars.slice(1).concat(girl_chars.slice(1));

    char_match = 0;
    for (let i = 0; i < word_chars.length; i++) {
        for (let j = 0; j < total_chars.length; j++) {
            if (word_chars[i] === total_chars[j]) {
                char_match++;
                total_chars.splice(j, 1);
                j = total_chars.length;
            }
        }
    }
